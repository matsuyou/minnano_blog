<html>
<head><title>記事一覧|みんなのブログ研究書</title>
<?php
...
<?php
   ...
   $sql = "select * from kizi order by num desc limit :start , :hyouzi";
   ...
   $fp = fopen("num.txt", "r"); //テキストファイルからnumの取得
   $num = fgets($fp) - 1;   //num = 総記事数
   fclose($fp);
   $page_size = ceil($num / $hyouzi); //表示数で割った数(繰り上がり)＝ページ数
...
<?php
   ...
   if ($page > 1){
      $PAGE = $page - 1;
      echo "<li><a href='list.php?page=".$PAGE."'><<前</a>";
      echo"</li>\n";
   }
   ...
   for ($x=$start; $x <= $start+8; $x++) {    //ページネーションリンク
      ...
      if($x === $page){
         echo "<b><a href='list.php?page=".$safe_x."'>".$safe_x."</a></b>";
      }else{
         echo"<a href='list.php?page=".$safe_x."'>".$safe_x."</a>";
      }
      echo"</li>\n";
   ...
   if ($page != $page_size){
      $PAGE = $page + 1;
      echo "<li><a href='list.php?page=".$PAGE."'>次>></a>";
      echo"</li>\n";
   }
